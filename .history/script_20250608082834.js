    ...
    if (analyzeButton && urlInput && window.electron && window.electron.analyzeUrl) {
      analyzeButton.addEventListener('click', async () => {
        ...
        try {
          ...
          // 统一拦截所有 cookies 失效相关英文提示（正则匹配，兼容各种变体、URL、换行、乱码、FAQ/Extractors结尾等）
          const cookiesInvalidPattern = /sign\s*in\s*to\s*confirm\s*you.*bot|sign\s*in\s*to\s*confirm\s*you.*robot|how\s*to\s*manually\s*pass\s*cookies|use\s*--cookies-from-browser|exporting-youtube-cookies|the\s*provided\s*youtube\s*account\s*cookies\s*are\s*no\s*longer\s*valid|yt-dlp\.wiki\/(extractors|faq)#exporting-youtube-cookies|yt-dlp\.wiki\/(extractors|faq)#how-do-i-pass-cookies-to-yt-dlp|\u4eba\u5de5\u9a8c\u8bc1|\u4eba\u673a\u9a8c\u8bc1/igms;
