      ...
      analyzeButton.addEventListener('click', async () => {
        ...
        try {
          ...
          if (videoFormats.length > 0) {
            formatList += '视频格式：\n' + videoFormats.map(f => f.desc + '（ID: ' + f.id + '）').join('\n') + '\n';
          }
          if (audioFormats.length > 0) {
            formatList += '音频格式：\n' + audioFormats.map(f => f.desc + '（ID: ' + f.id + '）').join('\n') + '\n';
          }
          contentTextarea.value = (info.description || '') + formatList;
          showToast('分析成功，已获取所有格式', 'success');
        } catch (e) {
          console.error('[analyzeUrl] 异常:', e);
          showToast('分析失败: ' + (e.message || e), 'error');
          if (contentTextarea) contentTextarea.value = '分析失败：' + (e.message || e);
        } finally {
