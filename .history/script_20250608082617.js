    ...
    if (analyzeButton && urlInput && window.electron && window.electron.analyzeUrl) {
      analyzeButton.addEventListener('click', async () => {
        ...
        try {
          ...
          // 统一拦截所有 cookies 失效相关英文提示（正则匹配，兼容各种变体）
          const cookiesInvalidPattern = /((sign in to confirm you.{0,40}bot)|(how to manually pass cookies)|(use --cookies-from-browser)|(exporting-youtube-cookies)|(the provided youtube account cookies are no longer valid))/i;
