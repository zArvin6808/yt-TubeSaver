    ...
    if (analyzeButton && urlInput && window.electron && window.electron.analyzeUrl) {
      analyzeButton.addEventListener('click', async () => {
        ...
        try {
          ...
          // 统一拦截所有 cookies 失效相关英文提示（正则匹配，兼容各种变体、URL、换行、乱码等）
          const cookiesInvalidPattern = /sign\s*in\s*to\s*confirm\s*you.*bot|how\s*to\s*manually\s*pass\s*cookies|use\s*--cookies-from-browser|exporting-youtube-cookies|the\s*provided\s*youtube\s*account\s*cookies\s*are\s*no\s*longer\s*valid|yt-dlp\.wiki\/extractors#exporting-youtube-cookies|yt-dlp\.wiki\/faq#how-do-i-pass-cookies-to-yt-dlp/igms;
