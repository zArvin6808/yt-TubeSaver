                ...
                if (window.electron && window.electron.validateCookies) {
                  window.electron.validateCookies(selectedCookiesPath).then(res => {
                    ...
                    if (res.valid) {
                      ...
                      if (res.error) errMsg += `\n详细信息：${res.error}`;
                      if (errorDetail) errorDetail.textContent = res.error || '';
                    ...
                    cookiesFileName.style.color = '#f44336';
                    const errorDetail = document.getElementById('cookiesErrorDetail');
                    if (errorDetail) errorDetail.textContent = err && err.message ? err.message : '';
      ...
      analyzeButton.addEventListener('click', async () => {
        ...
        try {
          ...
          if (info.formats && Array.isArray(info.formats)) {
            ...
            contentTextarea.value = table;
          } else {
            contentTextarea.value = '未获取到格式信息';
          }
          showToast('分析成功，已获取所有流编码信息', 'success');
        } catch (e) {
          console.error('[analyzeUrl] 异常:', e);
          // 递归提取所有可能的错误文本
          ...
          // 统一拦截所有 cookies 失效相关英文提示（正则匹配，兼容各种变体、URL、换行、乱码、FAQ/Extractors结尾等，支持所有特殊字符）
          const cookiesInvalidPattern = /sign\s*in\s*to\s*confirm\s*you[^\n]{0,100}(bot|robot|\u4eba\u5de5|\u4eba\u673a)|how\s*to\s*manually\s*pass\s*cookies|use\s*--cookies-from-browser|exporting-youtube-cookies|the\s*provided\s*youtube\s*account\s*cookies\s*are\s*no\s*longer\s*valid|yt-dlp\.wiki\/(extractors|faq)#exporting-youtube-cookies|yt-dlp\.wiki\/(extractors|faq)#how-do-i-pass-cookies-to-yt-dlp|\u4eba\u5de5\u9a8c\u8bc1|\u4eba\u673a\u9a8c\u8bc1|cookies?\s*无效|cookies?\s*失效/igms;
          if (cookiesInvalidPattern.test(msg)) {
            msg = 'cookies.txt（无效）';
            showToast(msg, 'error');
            if (contentTextarea) contentTextarea.value = msg;
            return; // 直接返回，彻底阻断后续英文内容显示
          }
          showToast('分析失败: ' + msg, 'error');
          if (contentTextarea) contentTextarea.value = '分析失败：' + msg;
        } finally {
