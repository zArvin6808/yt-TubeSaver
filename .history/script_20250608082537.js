    ...
    if (analyzeButton && urlInput && window.electron && window.electron.analyzeUrl) {
      analyzeButton.addEventListener('click', async () => {
        ...
        try {
          ...
          // 统一拦截所有 cookies 失效相关英文提示（忽略大小写和空格，兼容各种变体）
          const msgNorm = (msg || '').replace(/\s+/g, '').toLowerCase();
          if (
            msgNorm.includes('theprovidedyoutubeaccountcookiesarenolongervalid') ||
            msgNorm.includes('signintoconfirmyou') ||
            msgNorm.includes('howtomanuallypasscookies') ||
            msgNorm.includes('use--cookies-from-browser') ||
            msgNorm.includes('exporting-youtube-cookies') ||
            msgNorm.includes('signintoconfirmyou’renotabot') ||
            msgNorm.includes('signintoconfirmyou’renotarobot') ||
            msgNorm.includes('signintoconfirmyourebot') ||
            msgNorm.includes('signintoconfirmyouarenotabot')
          ) {
